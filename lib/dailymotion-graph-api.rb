...
module DailymotionGraphApi
  class Client
    ...
    def refresh_token
      token('refresh_token', auth: true)
    end

    def get_access_token(code, callback_url)
      token('authorization_code', auth: false, redirect_uri: callback_url, code: code)
    end
    ...
    def token(grant_type, args = {})
      ...
      r = send_request('/oauth/token', { verb: :post, auth: auth, body: {
